...
def train(forward_step_func, model, optimizer, opt_param_scheduler,
    ...
    from performance_evaluator import PerformanceEvaluator, format_numel_str

    gpt_model = model[0].module.module
    model_numel = sum(p.numel() for p in gpt_model.parameters())
    ...
    while iteration < args.train_iters:
        ...
        #                         config)
        # if torch.distributed.get_rank() == 0:
        #     prof.export_chrome_trace('/home/jiangmingyan/workspace/Megatron-LM/examples/GPT/gpt_profile.json')
        ...
        loss_dict, skipped_iter, grad_norm, num_zeros_in_grad = \
            train_step(forward_step_func,
                       ...
                       config)
        iteration += 1
        consumed_samples = mpu.get_data_parallel_world_size() * \
                                       args.micro_batch_size * \
                                       get_num_microbatches()

        args.consumed_train_samples += consumed_samples
        performance_evaluator.on_step_end({}, consum_sampels=consumed_samples)
